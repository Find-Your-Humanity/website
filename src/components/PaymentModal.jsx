	...
	const renderPaymentUI = async () => {
		try {
			if (typeof paymentWidget.renderPaymentMethods === "function") {
				...
				console.error(
					"❌ widget.renderPaymentMethods가 함수가 아닙니다"
				);
				throw new Error(
					"renderPaymentMethods 메서드를 찾을 수 없습니다"
				);
			}
		} catch (error) {
	...
	const handlePaymentRequest = async () => {
		...
		try {
			...
			if (error.message && error.message.includes("UNAUTHORIZED_KEY")) {
				errorMessage =
					"결제 시스템 인증에 실패했습니다. 관리자에게 문의해주세요.";
			} else if (
				error.message &&
				error.message.includes("NOT_REGISTERED_PAYMENT_WIDGET")
			) {
				errorMessage =
					"결제 UI가 등록되지 않았습니다. 관리자에게 문의해주세요.";
			}
